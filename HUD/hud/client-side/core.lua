...
CreateThread(function()
	...
	while true do
		if LocalPlayer.state.Active then
			...
			if MovimentSpeed > 0.0 and LocalPlayer.state.Active then
				local Ped = PlayerPedId()
				if not IsPedInAnyVehicle(Ped) then
					SetPedMoveRateOverride(Ped, 1.0 + MovimentSpeed)
					RestorePlayerStamina(PlayerId(), MovimentSpeed)
				end
			end
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- ADDSTATEBAGCHANGEHANDLER:REGION
-----------------------------------------------------------------------------------------------------------------------------------------
AddStateBagChangeHandler("Region",("player:%s"):format(LocalPlayer.state.Source),function(Name,Key,Value)
	SendNUIMessage({ Action = "Region", Payload = Value })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- ADDSTATEBAGCHANGEHANDLER:HOURS
-----------------------------------------------------------------------------------------------------------------------------------------
AddStateBagChangeHandler("Hours","global",function(Name,Key,Value)
	SendNUIMessage({ Action = "Clock", Payload = { Value, GlobalState.Minutes } })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- ADDSTATEBAGCHANGEHANDLER:MINUTES
-----------------------------------------------------------------------------------------------------------------------------------------
AddStateBagChangeHandler("Minutes","global",function(Name,Key,Value)
	SendNUIMessage({ Action = "Clock", Payload = { GlobalState.Hours, Value } })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- ADDSTATEBAGCHANGEHANDLER:ILLNESS
-----------------------------------------------------------------------------------------------------------------------------------------
AddStateBagChangeHandler("Illness",("player:%s"):format(LocalPlayer.state.Source),function(Name,Key,Value)
	SendNUIMessage({ Action = "Illness", Payload = Value })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddStateBagChangeHandler("Safezone",("player:%s"):format(LocalPlayer.state.Source),function(Name,Key,Value)
	SendNUIMessage({ Action = "Safezone", Payload = Value and { true,Value } or false })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- HUD:VOIP
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("Progress",function(Message,Timer)
	SendNUIMessage({ Action = "Progress", Payload = { Message, "", Timer - 300 } })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- HUD:THIRST
-----------------------------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- HUD:GIRDLEMOVIMENTSPEED
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("hud:GirdleMovimentSpeed")
AddEventHandler("hud:GirdleMovimentSpeed",function(Value)
	MovimentSpeed = tonumber(Value) or 0.0

	local Ped = PlayerPedId()
	if not IsPedInAnyVehicle(Ped) then
		SetPedMoveRateOverride(Ped, MovimentSpeed > 0.0 and (1.0 + MovimentSpeed) or 1.0)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- HUD:ADDGEMSTONE
